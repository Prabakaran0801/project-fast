...
export async function POST(req: NextRequest) {
  ...
  if (!parsed.success)
    return NextResponse.json(
      { error: "Invalid files", details: parsed.error.issues },
      { status: 400 },
    );

  const { files, emails, message, fromEmail, sendCopy } = parsed.data;
  const allRecipients = [...(emails || [])];
  if (sendCopy && fromEmail && !allRecipients.includes(fromEmail))
  ...
  if (s3Ready) {
    try {
      ...
      uploadUrls = await Promise.all(
        files.map(async (f) => {
          ...
          const url = await getSignedUrl(
            ...
            { expiresIn: 3600, checksumAlgorithm: undefined as any } as any,
          );
          return {
            filename: f.name,
            key,
      ...
      console.warn(
        "[transfer] presigned PUT failed, fallback to mock",
        String(e).slice(0, 200),
      );
      uploadUrls = files.map((f) => ({
        filename: f.name,
        key: `transfers/${transferId}/${f.name}`,
        uploadUrl: null,
      }));
    }
  } else {
  ...
  try {
    ...
    console.log(
      `[transfer] created ${transferId} with ${files.length} files, presigned ${uploadUrls.length} URLs sender=${fromEmail}`,
    );
  } catch (e) {
    console.warn(
      "[transfer] DB create failed, using mock",
      String(e).slice(0, 200),
    );
  }

  // Send emails via Nodemailer — receiver MailTransfer style + sender confirmation
  ...
  if (allRecipients.length) {
    ...
    if (fromEmail && !allRecipients.includes(fromEmail)) {
      try {
        const c = await sendSenderConfirmation({
          to: fromEmail,
          transferUrl,
          files: emailFiles,
          expiresAt,
          recipient: emails![0] || allRecipients[0],
        });
        emailResults.push({
          to: fromEmail,
          status: "sent",
          note: "sender confirmation",
          messageId: (c as any).messageId,
        });
      } catch (e) {
        emailResults.push({
          to: fromEmail,
      ...
      emailResults.push({
        to: fromEmail,
        status: "sent",
        note: "sender confirmation (already recipient)",
      });
    }
  }

  console.log(
    `[transfer] ${transferId} emailResults:`,
    emailResults.map((r: any) => `${r.to}:${r.status}`).join(", "),
  );
  return NextResponse.json({
    transferId: dbId,
    transferUrl,
